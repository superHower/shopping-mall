...
export default {
  ...
  methods: {
    goSearch (key) {
      ...
      if (index !== -1) { // key在history中
        this.history.splice(index, 1)// splice(从哪开始, 删除几个, 项1, 项2)
      }
      this.history.unshift(key) // 把key添加到history最前面
      setHistoryList(this.history) // 更新historyList
